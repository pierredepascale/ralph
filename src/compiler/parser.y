...
%type <nothing> dylan_program 
%type <nothing> semi_opt
...
%type <token> variable_name variable_name_opt any_variable_name_except_function
...
%type <binop> binop
%type <arglist> arguments_opt arguments values
...
/*    |   values
      { $$ = make_expr_constituent(make_function_call(make_varref(make_id(make_token("values", 6))), $1)) ; } */
    |   error			{ $$ = make_error_constituent(); }
;

defining_form:
        CLASS class_definition
        { $$ = set_class_flags(0, $2) ; free($1) ; }
    |	GENERIC generic_function_definition
	{ free($1); $$ = set_generic_flags(0, $2); }
    |   DEF named_method
        { free($1); $$ = make_define_method(0, $2) ;}
    |	DEF bindings
	{ $$ = make_define_variable($1->line, $2); free($1); }
...
values:
        values COMMA expression
        { free($2) ; $$ = add_argument($1, $3) ; }
    |   expression
        { $$ = add_argument(make_argument_list(), $1) ; }
;
...
slot_spec:
        variable_name slot_type_opt slot_init_expr_opt
        {
          int line = $1->line;
          $$ = make_slot_spec(line, 0, alloc_INSTANCE, $1 ? make_id($1) : NULL,
                              $2, $3, make_property_list()) ;
        }
;
...
anonymous_method:
	METHOD method_description END
	{ free($3); $$ = set_method_source($1, $2); }
    |   FN method_description END
        { free($3) ; $$ = set_method_source($1, $2); }
;

named_method:
	variable_name method_description END
	{ struct id *id = make_id($1);
	  free($3);
...
module_definition:
	variable_name module_clauses_opt END
	{ free($3);
...
library_definition:
	variable_name library_clauses_opt END
	{ free($3);
	  $$ = (struct constituent *) set_namespace_name($2, $1);
	}
;
